...
const Admin = () => {
  ...
  const fetchProjects = async (): Promise<Project[]> => {
    const response = await fetch("https://cvbackend-rj6m.onrender.com/api/projects");
    if (!response.ok) {
  ...
  const updateProjectMutation = useMutation({
    mutationFn: async ({ id, updatedData }: { id: string; updatedData: Partial<Omit<Project, '_id' | 'createdAt' | 'updatedAt'>> }) => {
      const response = await fetch(`https://cvbackend-rj6m.onrender.com/api/projects/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteProjectMutation = useMutation({
    mutationFn: async (projectId: string) => {
      const response = await fetch(`https://cvbackend-rj6m.onrender.com/api/projects/${projectId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
        const errorData = await response.json();
  ...
  const fetchBlogPosts = async (): Promise<BlogPost[]> => {
    const response = await fetch("https://cvbackend-rj6m.onrender.com/api/blog");
    if (!response.ok) {
  ...
  const updateBlogPostMutation = useMutation({
    mutationFn: async ({ id, updatedData }: { id: string; updatedData: Partial<Omit<BlogPost, '_id' | 'createdAt' | 'updatedAt'>> }) => {
      const response = await fetch(`https://cvbackend-rj6m.onrender.com/api/blog/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(updatedData),
      });
  ...
  const deleteBlogPostMutation = useMutation({
    mutationFn: async (blogPostId: string) => {
      const response = await fetch(`https://cvbackend-rj6m.onrender.com/api/blog/${blogPostId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
        const errorData = await response.json();
